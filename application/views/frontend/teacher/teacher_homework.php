...
(function () {
	...
	function add(){if(addInFlight){return;}var subjectId=parseInt(document.getElementById('th_subject_id').value||0,10);if(!subjectId){msg('Please select subject.',true);return;}var f=document.getElementById('th_pdf').files[0];var desc=document.getElementById('th_desc').value.trim();if(!desc&&!f){msg('Enter a description and/or choose a PDF.',true);return;}var fd=new FormData();fd.append('access_token',token);fd.append('batch_id',batchId);fd.append('subject_id',subjectId);fd.append('date',document.getElementById('th_date').value);fd.append('description',desc);if(f){fd.append('pdf_file',f);}setAddBusy(true);fetch(addUrl,{method:'POST',headers:{'Authorization':'Bearer '+token},body:fd}).then(function(r){return r.json();}).then(function(j){var ok=!!(j&&(j.status==='true'||j.status===true));msg((j&&j.msg)||'Added',!ok);if(ok){document.getElementById('th_subject_id').value='';document.getElementById('th_pdf').value='';document.getElementById('th_desc').value='';}load();setAddBusy(false);},function(){msg('Could not add homework.',true);setAddBusy(false);});}
	document.addEventListener('DOMContentLoaded',function(){document.getElementById('th_date').value=(new Date()).toISOString().slice(0,10);addBtn=document.getElementById('th_add');addBtnDefaultText=addBtn?addBtn.textContent:'Add homework';if(addBtn){addBtn.addEventListener('click',add);}loadSubjects();load();});
})();
</script>
