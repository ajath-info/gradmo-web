...
(function () {
	...
	function setAddBusy(busy){if(!addBtn){return;}addInFlight=!!busy;addBtn.disabled=!!busy;addBtn.textContent=busy?'Uploading...':addBtnDefaultText;setLoader(!!busy);}
	...
	function add(){if(addInFlight){return;}var fd=new FormData();fd.append('access_token',token);fd.append('batch_id',batchId);fd.append('title',document.getElementById('tn_title').value.trim());var s=document.getElementById('tn_subject');fd.append('subject',s && s.value ? s.options[s.selectedIndex].text : '');fd.append('topic',document.getElementById('tn_topic').value.trim());var f=document.getElementById('tn_file').files[0];if(f){fd.append('pdf_file',f);}setAddBusy(true);fetch(addUrl,{method:'POST',headers:{'Authorization':'Bearer '+token},body:fd}).then(function(r){return r.json();}).then(function(j){var ok=!!(j&&(j.status==='true'||j.status===true));msg((j&&j.msg)||'Uploaded',!ok);if(ok){document.getElementById('tn_title').value='';document.getElementById('tn_subject').value='';document.getElementById('tn_topic').value='';document.getElementById('tn_file').value='';}load();setAddBusy(false);},function(){msg('Could not upload note.',true);setAddBusy(false);});}
	document.addEventListener('DOMContentLoaded',function(){addBtn=document.getElementById('tn_add');addBtnDefaultText=addBtn?addBtn.textContent:'Upload note';if(addBtn){addBtn.addEventListener('click',add);}loadSubjects();load();});
})();
</script>
