...
<div class="inst-detail-page">
	<div class="inst-detail-container">
		<div class="inst-detail-panel">
			...
			<div id="tn_progress" class="inst-upload-progress is-hidden" aria-live="polite">
				<div class="inst-upload-progress-bar"><span id="tn_progress_fill" class="inst-upload-progress-fill"></span></div>
				<div id="tn_progress_text" class="inst-upload-progress-text">Preparing upload...</div>
			</div>
...
(function () {
	...
	function setAddBusy(busy){if(!addBtn){return;}addInFlight=!!busy;addBtn.disabled=!!busy;addBtn.textContent=busy?'Uploading...':addBtnDefaultText;setLoader(!!busy);if(busy){setProgress(0,'Preparing upload...');}else{resetProgress();}}
	...
	function add(){if(addInFlight){return;}var fd=new FormData();fd.append('access_token',token);fd.append('batch_id',batchId);fd.append('title',document.getElementById('tn_title').value.trim());var s=document.getElementById('tn_subject');fd.append('subject',s && s.value ? s.options[s.selectedIndex].text : '');fd.append('topic',document.getElementById('tn_topic').value.trim());var f=document.getElementById('tn_file').files[0];if(f){fd.append('pdf_file',f);}setAddBusy(true);uploadFormData(addUrl,fd).then(function(j){var ok=!!(j&&(j.status==='true'||j.status===true));msg((j&&j.msg)||'Uploaded',!ok);if(ok){document.getElementById('tn_title').value='';document.getElementById('tn_subject').value='';document.getElementById('tn_topic').value='';document.getElementById('tn_file').value='';}load();setAddBusy(false);},function(){msg('Could not upload note.',true);setAddBusy(false);});}
	document.addEventListener('DOMContentLoaded',function(){addBtn=document.getElementById('tn_add');addBtnDefaultText=addBtn?addBtn.textContent:'Upload note';progressWrap=document.getElementById('tn_progress');progressFill=document.getElementById('tn_progress_fill');progressText=document.getElementById('tn_progress_text');if(addBtn){addBtn.addEventListener('click',add);}loadSubjects();load();});
})();
</script>
