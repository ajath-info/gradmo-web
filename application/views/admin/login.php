    ...
    loginBtn.addEventListener('click', function () {
        ...
        fetch(frm.getAttribute('action') || '', {
            ...
            try {
                ...
                if (typeof toastr !== 'undefined') { toastr.error('Login failed. Invalid server response.'); }
                return;
            }
            if (resp && (resp.status === '1' || resp.status === 1 || resp.status === true || resp.status === 'true')) {
                if (resp.url) {
            ...
            if (typeof toastr !== 'undefined') {
                toastr.error((resp && (resp.msg || resp.message)) ? (resp.msg || resp.message) : 'Invalid credentials');
            }
        }).catch(function () {
